...
public class PasswordServiceImpl implements PasswordService {
     ...
     * @param connectedUser  Đối tượng Principal đại diện cho người dùng đang đăng nhập.
     ...
     */
    @Override
    public boolean changePassword(PasswordChangeRequest request, Principal connectedUser) {
    ...
    public String sendOTPEmail(String username) throws MessagingException {
        User user = userRepository.findByUsernameOrGmail(username, username)
                .orElseThrow(() -> new UserNotFoundException("Username or gmail is not found : " + username));
        if (user.getAccountProvider().name().equals(AccountProvider.LOCAL.name())
            ...
            final int expirationTime = 15; // 15 minutes
...
//            emailService.send(email, emailTemplate.getSubject(), emailTemplate.generateEmailBody());
    ...
    public String verifyOTP(String username, String otp) {
        ...
        if (passwordReset != null) {
            ...
            if (checkOtpValue) {
                final int ExpirationToken = 15;// 15m
